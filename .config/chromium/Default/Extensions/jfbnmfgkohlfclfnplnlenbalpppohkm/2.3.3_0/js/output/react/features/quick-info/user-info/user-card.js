class UserInfoWidgetUserCard extends React.Component {
  constructor(props) {
    ...
    this.state = {
      user: props.user,
      premiumIcon: React.createElement("span", null),
  ...
  loadPremiumIcon(userId) {
    ...
    RPlus.premium.getPremium(userId).then(premium => {
      if (this.props.user.id === userId && premium) {
        ...
        this.setState({
          premiumIcon: React.createElement("span", {
  ...
  renderLabels() {
    ...
    }, "Collectibles: ", global.addCommas(this.state.inventoryData.collectibles.length)), React.createElement("div", {
      class: "avatar-card-label"
  ...
  render() {
    ...
    })), React.createElement(ThumbnailPresence, {
